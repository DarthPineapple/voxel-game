...
void Pipeline::createPipeline(const std::string& vertShaderPath, const std::string& fragShaderPath) {
    ...
    // Vertex input configuration for Mesh vertices
    VkVertexInputBindingDescription bindingDescription{};
    bindingDescription.binding = 0;
    bindingDescription.stride = sizeof(float) * 8; // 3 (position) + 3 (normal) + 2 (texCoord)
    ...
    // Use clockwise because projection matrix flips Y (see camera.cpp line 124)
    // This converts counter-clockwise wound faces in world space to clockwise in NDC
